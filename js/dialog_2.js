...
function showDialog( title, subtext, buttons, callback, _checked, _focused, cls, chained )
{
	...
	if( !document.getElementById("dialog") ){
		var dialog = createClassDiv("", "", "dialog");
		dialog.className = "hide";
		dialog.innerHTML = '<div id="dialogWrapper" class="wrapper"></div><div id="dialog_logo"></div><div id="dialog_time">18:45</div>';
		document.body.appendChild(dialog);
		//document.body.appendChild( '<div id="dialog" class="hide"><div id="dialogWrapper" class="wrapper"></div><div id="dialog_logo"></div><div id="dialog_time">18:45</div></div>' );
   ...
   // $.each( arguments, function(i, arg){
   	arguments.forEach(function(i, arg){
    ...
    document.getElementById("dialog").removeClass();
    
	dialog.options = dialog.buttons.length; 
	
	document.getElementById("dialog").removeClass("hide");
	document.getElementById("dialog").addClass("show");
    
	console.log("set class " + dialog.className);
	
	if(dialog.className && typeof(dialog.className) == "string"){
		document.getElementById("dialog").addClass(dialog.className);
	}
	...
	document.getElementById("dialog").html("");

    if(!document.getElementById("dialog").hasClass("basicInfoDialog")){

        document.getElementById("dialog").appendChild("<div class=\"dialogControls\"></div>");
        document.getElementsByClassName("dialogControls")[0].appendChild("<div class=\"dialogTitle\">"+dialog.title+"</div>");
        document.getElementsByClassName("dialogControls")[0].appendChild("<div class=\"dialogText\">"+dialog.subtext+"</div>");
        document.getElementsByClassName("dialogControls")[0].appendChild(up_arrow);
		document.getElementsByClassName("dialogControls")[0].appendChild('<div id="dialogWrapper" class="wrapper"></div>');
		document.getElementsByClassName("dialogControls")[0].appendChild(down_arrow);
    }
	...
	document.getElementById("dialog").appendChild("<div id=\"dialog_logo\"></div>");
	document.getElementById("dialog").appendChild("<div id=\"dialog_time\">--:--</div>");

	var dialogClockUpdate = function(){
		if( document.getElementById("dialog_time") ){
			var d = new Date();
			//var timenow = prepend( d.getHours(),0,"2" ) + ":" + prepend( d.getMinutes(),0, 2);
			var timenow = ("0"+d.getHours()).slice(-2) + ":" + ("0"+d.getMinutes()).slice(-2); 
			
			console.log(timenow);
			document.getElementById("dialog_time").html( timenow );
		}
		else{
	...
	documet.getElementById("dialogWrapper").appendChild(dialogButtons);

	if(dialog.buttons.length > 0){
		dialog.buttons.forEach(function(i, label){
		...
		var buttonElems = document.getElementsByClassName("dialogButton")[0];
		var firstvisible = buttonElems[0];
		var lastvisible = null;
		var firstvisibleIdx = 0;
		if(dialog.focused >= dialog.visibleItems){
			buttonElems.forEach(  function(i, elem){
		...
		var scrolltop = (document.getElementsByClassName("dialogButton:eq("+ dialog.focused +")").outerHeight(true) * firstvisibleIdx);
		scrolltop += (firstvisibleIdx > 0) ? (dialogButtonMarginBottom*firstvisibleIdx) : 0;
		console.log("scrolltop: ", scrolltop);
		document.getElementById("dialogWrapper").scrollTop(scrolltop);
	}
...
function handleDialogArrows(){
	...
	if(document.getElementById("dialogWrapper").scrollTop() > 0){
	...
	if(document.getElementById("dialogWrapper")[0].scrollHeight > document.getElementById("dialogWrapper")[0].offsetHeight){
		down_arrow.removeClass("hide");
	}
	if(document.getElementById("dialogWrapper").scrollTop() < document.getElementById("dialogWrapper")[0].scrollHeight - document.getElementById("dialogWrapper")[0].offsetHeight){
		down_arrow.removeClass("hide");
	}
	else{
		down_arrow.addClass("hide");
	...
	switch(keyCode)
	{
		...
		case VK_UP:
			if( keyCode == VK_UP && dialog.focused > 0 ){
				var dialogWrapper = document.getElementById("dialogWrapper")[0];
				var prev = document.getElementsByClassName("dialogButton:eq("+ dialog.focused +")").prev();
				if(prev && prev[0].offsetTop < dialogWrapper.scrollTop){
					animating = true;
					document.getElementById("dialogWrapper").animate(
				...
				var dialogWrapper = document.getElementById("dialogWrapper")[0];
				var next = document.getElementsByClassName("dialogButton:eq("+ dialog.focused +")").next();
			...
			document.getElementsByClassName("dialogButton.focused")[0].removeClass("focused");
			document.getElementsByClassName("dialogButton:eq("+ dialog.focused +")").addClass("focused");
			break;
...
function hideDialog(){
    ...
    document.getElementById("dialog").html("");
    document.getElementById("dialog").removeClass("show");
    document.getElementById("dialog").addClass("hide");
    if( typeof( dialog.onClose ) == "function" ){
        dialog.onClose();
        dialog.onClose = null;
    }
