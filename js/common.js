var ENABLE_CONSENTFRAME=0, ENABLE_CONSENT = 0, ENABLE_SPONSOR = 0, ENABLE_MIDDLE =1 , VIDEO_PREVIEW = 1, ENABLE_WATCHED=1;
...
function moves(tit) {
	...
	if (GLOBALS.smid)
		sm = '&uid=' + GLOBALS.smid;
	llog('[moves] tit: '+ tit);
	piwik.src = "http://smarttv.anixa.tv/cretetv/DEV/move.php?path="+ encodeURIComponent(tit) +"&smid="+sm;
	return true;
}
...
if(GLOBALS.dev){
window.location.href = "http://smarttv.anixa.tv/cretetv/DEV/home/index.php?sd=cretetv&s=cretetv";
return true;
}
...
if(elem.obj.idnam == 'epg' && m.isOpen){
...
SceneManager.prototype.goBack = function () {
	...
	if (elem.obj.idnam == "cretetv") {
		...
		if(!GLOBALS.focusmgr.getObject('side-menu').isOpen) GLOBALS.focusmgr.focusObject("cat-list-" + o.focusedId, true);
		o.scrollToFocused(-1);
		activeCont = elem.obj;
		return true;
	}
...
function initApp() {
	...
	var pageid = 46;
	GLOBALS.smid = smid;
	if (1 && (location.host == "127.0.0.1" || location.href.indexOf('DEV') > 0)) {
