...
function createCookie(name, value, lifetime) {
	try {
		lifetime = lifetime || (365 * 24 * 60 * 60 * 1000); // Default 1 year
		var date = new Date();
		...
		debug("[COOKIE] Created cookie: " + cookieString); // ✅ Log here for debugging
	} catch (e) {
		debug("[COOKIE] Error creating cookie: " + e.message);
	}
}
...
function readCookie(name) {
  ...
  for (var i = 0; i < ca.length; i++) {
    ...
    if (c.indexOf(nameEQ) == 0) return c.substring(nameEQ.length, c.length);
  	...
  	debug('[COOKIE] Cookie not found: ${name}');
