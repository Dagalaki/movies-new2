...
//$file = '/var/log/nginx/access.log.2';
...
foreach ($hits as $s) {
	...
	if ($ins) {
		...
		if (!$readonly) {
			$mysqli->query("INSERT INTO durations_n SET id = '". $id ."', ts = ". $ts .", ip = '". md5($ip) ."', end = ". $ts .", smart_id = ". $smartId ." ON DUPLICATE KEY UPDATE end = ". $ts .", smart_id = ". $smartId);
			if ($doRep) $dupmysqli->query("INSERT INTO durations SET id = '". $id ."', ts = ". $ts .", ip = '". md5($ip) ."', end = ". $ts .", smart_id = ". $smartId ." ON DUPLICATE KEY UPDATE end = ". $ts .", smart_id = ". $smartId);
		}
		if ($mysqli->error)
