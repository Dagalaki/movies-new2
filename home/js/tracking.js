(function () {
  var LOG_EVENT_TYPE = {HB_REQ: 1, HB_RES: 2, HB_ERR: 3, HB_BOFF: 4, S_STRT: 5, S_STOP: 6, SE_UPDATE_START: 7, SE_UPDATE_STOP: 8, SE_UPDATE: 9, SE_SEND: 10};
	var HEARTBEAT_INTERVAL=1000, RESOLUTION= 2/* hd/sd*/, MAX_ERROR_COUNT= 4, INITIALIZE_SUSPENDED= false, CONSENT=false, MAX_ERROR_BACKOFF=3, TRACKING_GLOBAL_OBJECT='tgo', NEW_SESSION='', DELIVERY=3;
  var logQueue = [];
  ...
  g._cb = {};
  g._hb = 'http://smarttv.anixa.tv/tv-insight/req/?';
  g._cid = 'cid=15';
  g._h = '&r='+ RESOLUTION +'&d='+ DELIVERY +'&t=';
  g._did = '123';
  g._sid = 'ses-'+Math.random();
  ...
  g._beat = function () {
    try {
      ...
      delay = 1;
	    var src = g._hb + g._cid + g._h + Date.now();// + '/{{PIXEL_NAME}}?f={{HEARTBEAT_INTERVAL}}');
	    NEW_SESSION= g._hb;
	    //console.log(src);
      hbImg.setAttribute('src', src);
  ...
  function uploadSessionEnd (sid, ts, retries, successCB, errorCB) {
    try {
      ...
      seImg.setAttribute('src', g._hb + sid + 't=' + ts);
    } catch(e) {}
  };
